...
import { NodeUsersList } from "./NodeUsersList";
import {
    usePluvBroadcast,
    usePluvMyPresence,
    usePluvMyself,
...
export type NodeChatRoomProps = Record<string, never>;

export const NodeChatRoom: FC<NodeChatRoomProps> = () => {
    ...
    return (
        <div className={clsx("flex flex-row items-stretch")}>
            {connectionIds.map((connectionId) => (
                <NodeCursor key={connectionId} connectionId={connectionId} />
            ))}
            ...
            <NodeUsersList className={clsx("rounded-r-md border-gray-600")} />
        </div>
    );
};
