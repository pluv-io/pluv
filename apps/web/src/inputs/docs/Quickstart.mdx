...
```bash
# For the server
npm install @pluv/io @pluv/platform-node yjs
...
Define a io (websocket client) instance on the server codebase:
...
* Usage with Cloudflare Workers (server-side)
* Using CRDT storage with yjs
