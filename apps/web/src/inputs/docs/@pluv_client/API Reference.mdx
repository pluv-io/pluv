...
const room = client.createRoom("my-example-room", {
    // If your `PluvIO` implements auth, you will need to specify an
    // `authEndpoint` on your room.
...
const room = client.createRoom("my-example-room", {
    // If you need to use a POST request or specify additional headers,
    // you can return an object containing options for `fetch`.
    authEndpoint: (room: string) => ({
        url: "/api/room",
