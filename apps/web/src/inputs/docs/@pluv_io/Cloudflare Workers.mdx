...
export const io = createIO({
    platform: platformCloudflare<Env>({
        /**
         * Specify whether WebSocket event listeners should be "attached" to or
         * "detached" from the WebSocket upon registration.
         * 
...
export class RoomDurableObject implements DurableObject {
    ...
    constructor(state: DurableObjectState, env: Env) {
        this._room = ioServer.getRoom(state.id.toString(), { env, state });
    }

    // Only needed if using "detached" mode (i.e. hibernation)
    public webSocketClose(ws: WebSocket, code: number, reason: string): void | Promise<void> {
