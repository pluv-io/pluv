...
React provider component to mock `PluvRoomProvider`. Enables hooks from `CreateRoomBundle` while completely offline. This can be useful as in [Storybook](https://storybook.js.org/) within a decorator when mocking `PluvRoomProvider`.
