...
Create a `PluvClient` that will create rooms to connect to your server's `PluvIO` instance.
...
Create a `PluvRoom` that websockets can join and leave. The second argument is a configuration for presence and storage.
...
Establishes a websocket connection with a room. If the `PluvClient` specifies an `authEndpoint`, the endpoint will be called when this function is called.
