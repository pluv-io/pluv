...
export const {
    ...
    // Endpoint if your server specifies pluv authentication
    authEndpoint: ({ metadata, room }) => `https://${metadata.myUrl}/room/${room}`,
    /**
     * @optional
     * @description Define the initial storage for the room, as well as
     * the CRDT to use
     */
    ...
    /**
     * @optional
     * @description Define metadata used to specify your endpoints. This
     * is particularly useful for Cloudflare Pages specifically
     */
    metadata: z.object({
        myUrl: z.string(),
    }),
    ...
    wsEndpoint: ({ metadata, room }) => `wss://${metadata.myUrl}/room/${room}`,
});
```
...
React provider component to mock `PluvRoomProvider`. Enables hooks from `CreateRoomBundle` while completely offline. This can be useful in [Storybook](https://storybook.js.org/) within a storybook decorator when mocking `PluvRoomProvider`.
...
const MyComponent: FC = () => {
    return (
        <PluvRoomProvider
            ...
            /**
             * @description This is only required if metadata is specified in
             * your `createRoomBundle`
             */
            metadata={() => ({
                myUrl: "www.pluv.io/api"
            })}
            /**
             * @optional
