...
description: "Get started with pluv.io on Next.js using the hosted pluv.io network for realtime WebSockets."
---
...
You can create a new project from the [pluv.io dashboard](https://pluv.io/app) by clicking on "**Add project**" from the sidebar.

<img className="mx-auto" width="100%" src="https://raw.githubusercontent.com/pluv-io/pluv/refs/heads/main/assets/create_project_dashboard.png" alt="Create project from the sidebar" loading="lazy" />
...
<img className="mx-auto" width="72%" src="https://raw.githubusercontent.com/pluv-io/pluv/refs/heads/main/assets/create_project_dialog.png" alt="Create project from the sidebar" loading="lazy" />
...
<img className="mx-auto" width="100%" src="https://raw.githubusercontent.com/pluv-io/pluv/refs/heads/main/assets/create_api_keys.png" alt="Create api key" loading="lazy" />

### Project Webhooks

On `@pluv/platform-pluv`, server event listeners (i.e. `getInitialStorage`, `onRoomDestroyed`, `onStorageDestroyed`, `onUserConnected` and `onUserDisconnected`) are webhooks that require a webhook secret key to use. To create this, navigate to the **Webhooks** page, from the sidebar of the project dashboard, and click on the "**Create webhook**" button. This webhook secret key must be copied and saved as an environment variable for your app.

<img className="mx-auto" width="100%" src="https://raw.githubusercontent.com/pluv-io/pluv/refs/heads/main/assets/create_webhook_dashboard.png" alt="Create webhook dashboard" loading="lazy" />
...
<img className="mx-auto" width="72%" src="https://raw.githubusercontent.com/pluv-io/pluv/refs/heads/main/assets/create_webhook_dialog.png" alt="Create webhook dialog" loading="lazy" />
...
export const ioServer = io.server({
  // Only if you're using storage (`treaty.storage`)
  ...
  // Optional: Only if you want to run code when storage is destroyed,
  // such as saving the last storage state. Only fires when storage was
  // initialized via initializeSession
  onStorageDestroyed: async ({ room, encodedState, context }) => {
...
`@pluv/platform-pluv` requires setting up authorization unlike in the other platforms where it is optional. Define an endpoint to create and return an access token for the user that has access to the room. See [Authorization](/docs/guides/authorization) for more detailed documentation.
...
export const ioServer = io.server({
  ...
  // Optional: This handler is called when a room is destroyed. Use for
  // cleanup if needed. This event does NOT include encodedState to prevent
  // accidentally saving empty or uninitialized storage data.
  onRoomDestroyed: async ({ room }) => {
    // Room destroyed - use for cleanup if needed
  },
  // Optional: This handler is called when storage is about to be destroyed.
