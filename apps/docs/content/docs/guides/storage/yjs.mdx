...
To start, put a `yjs.schema` on the shared [treaty](/docs/guides/treaty) and pass that treaty to `createIO`. `getInitialStorage` is required when `treaty.storage` is set.
...
Then put the `yjs.schema` on the shared [treaty](/docs/guides/treaty) and pass that treaty to `createClient`. JSON seeds go in `initialStorage`.
...
export const { PluvRoomProvider, useStorageField } = bundle;
...
Finally, we can then read and write to our [Yjs shared-type](https://docs.yjs.dev/getting-started/working-with-shared-types) via the `useStorageField` hook we exported earlier.

```tsx
// frontend/GroceryList.tsx

import type { FC } from "react";
import { useStorageField } from "./pluv";

export interface GroceryListProps {}

export const GroceryList: FC<GroceryListProps> = () => {
  const [groceries, yArray] = useStorageField("groceries");
...
import { PluvRoomProvider, useStorageField } from "./pluv";
...
const [chats, yMap] = useStorageField("chats");
...
const provider = useMemo(() => {
  return yjs.provider({
    room,
    // Specify which treaty presence field to read/write Yjs awareness to.
    // The name is yours; it must match a key on your treaty presence schema.
