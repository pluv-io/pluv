...
```bash
# pluv.io packages
npm install @pluv/crdt @pluv/crdt-loro
...
Then put the `loro.schema` on the shared [treaty](/docs/guides/treaty) and pass that treaty to `createClient`. JSON seeds go in `initialStorage`.
...
export const { PluvRoomProvider, useStorageField, useTransact } = bundle;
...
Finally, we can then read and write to our [Loro container type](https://loro.dev/docs/tutorial/loro_doc#container-types) via the `useStorageField` hook we exported earlier.
...
export const GroceryList: FC<GroceryListProps> = () => {
  const [groceries, loroList] = useStorageField("groceries");
...
import { useStorageField, useTransact } from "./pluv";

const transact = useTransact();

// "groceries" is a key from the root properties of `loro.schema`
...
It isn't so uncommon to want to create a nested structure containing multiple Loro container types. Declare the nested shape on `loro.schema`. JSON seeds stay JSON. Nested CRDT nodes are constructed from that seed at hydrate time.

To declare a top-level container type on the `LoroDoc`, use `loro.loroList`, `loro.loroMap`, `loro.loroText`, and the other schema helpers on `loro.schema`. JSON-only leaves use `s` from `@pluv/crdt`.
...
Then, to insert a nested container type at runtime, use the `@pluv/crdt-loro` helpers (or the native Loro constructors). These return native Loro types.
...
import { PluvRoomProvider, useStorageField, useTransact } from "./pluv";
...
const transact = useTransact();
// "chats" is a key from the root properties of `loro.schema`
// loroMap is a container-type from Loro
const [chats, loroMap] = useStorageField("chats");
