...
Then define a shared [treaty](/docs/guides/treaty) with Yjs storage and presence for BlockNote:

1. Storage: a [Yjs.XmlFragment](https://docs.yjs.dev/api/shared-types/y.xmlfragment) for editor contents.
2. Presence: a field for BlockNote awareness (we recommend `z.any()`; the shape is internal to BlockNote).
...
import { useRoom, useStorageField } from "./pluv";
...
export const BlockNoteEditor: FC<BlockNoteEditorProps> = () => {
  const [, fragment] = useStorageField("myEditor");
...
That's all you need to enable realtime collaboration on BlockNote! From here on, you can further customize your BlockNote editor by following the official [BlockNote documentation](https://www.blocknotejs.org/docs). And you can persist the contents of your BlockNote editor by following pluv.io's documentation for [Loading Storage](/docs/guides/storage/loading).
