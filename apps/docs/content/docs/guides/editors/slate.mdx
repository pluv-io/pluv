...
```bash
# Slate packages
npm install slate slate-dom slate-react @slate-yjs/core
...
> **Note**: `slate-react` requires `slate-dom` as a peer dependency. Keep `slate`, `slate-dom`, and `slate-react` on the same minor version (for example, all `0.124.x`).
...
To start, we will need at least a basic Slate editor. The example below uses local React state, which is fine for a non-collaborative editor. In the collaborative setup below, Yjs owns the document instead, so that pattern is replaced.
...
Define a shared [treaty](/docs/guides/treaty) with a [Yjs.XmlText](https://docs.yjs.dev/api/shared-types/y.xmltext) for Slate's editor contents. Xml types are not JSON-seedable. Declare `yjs.yXmlText()` on the treaty and initialize the empty document when binding the editor (see `normalizeNode` below, or `slateNodesToInsertDelta` from `@slate-yjs/core` at connect time).
...
export const SlateEditor: FC<SlateEditorProps> = () => {
  const [, xmlText] = useStorageField("myEditor");
...
> **Note**: With Yjs, the shared `Y.XmlText` is the source of truth for the document. The `initialValue` prop is only used to initialize the editor on first mount. It is not controlled React state, and you should not mirror the editor value with `useState` and `onChange`.

That's all you need to enable realtime collaboration on Slate! From here on, you can further customize your Slate editor by following the official [Slate documentation](https://docs.slatejs.org). And you can persist the contents of your Slate editor by following pluv.io's documentation for [Loading Storage](/docs/guides/storage/loading).
