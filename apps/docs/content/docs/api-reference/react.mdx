---
title: "@pluv/react"
description: "API reference for @pluv/react: createBundle, PluvRoomProvider, and hooks for events, presence, storage, and history."
---
...
By default, storage hooks return `null` until CRDT storage is loaded. Pass `{ suspense: true }` to make them wait instead.

Wrap the components that use those hooks in `<Suspense>`. This applies to `useStorage`, `useStorageField`, `useDoc`, `useTransact`, and the undo/redo hooks. Presence and connection hooks do not wait. If the room closes or storage becomes unavailable while a hook is waiting, the nearest error boundary handles the rejection.

```ts
const { useStorageField } = createBundle(io, { suspense: true });
```
...
React provider component to enable `CreateBundle` hooks within child components. Mounting this component connects to a real-time room with other connected users. Unmounting this component will disconnect the room. See [Create Rooms](/docs/guides/rooms/react) for more details.
...
Fires a callback whenever the room reports a failure (procedure throw, size limit, rejected register). Not a stored last-error value.
...
const userId = useMyself(({ data }) => data.id);
```
...
Returns the user-info object for another person by `data.id`.

```tsx
const other = useOther(userId);

const selectionId = useOther(userId, (other) => other.presence.selectionId);
```
...
Returns all user-info objects for other people in the room. This may trigger a lot of re-renders if presence updates frequently.

> **Note**
> Consider using this hook only for deriving others' `data.id` values to pass into `useOther`.

```tsx
const others = useOthers();

const userIds = useOthers((others) => {
  return others.map(({ data }) => data.id);
});
```
...
For more information see: [History](/docs/guides/history)
...
Live socket count (`connectionCount`) and unique-user count (`userCount`). Pass a selector to subscribe to a slice.
...
Returns the room presence procedure proxy (`room.presence`). Do not destructure it.
...
Returns the room storage procedure proxy (`room.storage`). Do not destructure it. Procedures throw until CRDT storage is loaded.
...
Returns `[TData | null, SharedType | null]`

Returns a base-level property of our CRDT storage as a serialized JSON value and the native shared type (for example `Y.Array`). The component is re-rendered whenever the shared type is updated. The returned values are null while the room is still connecting or
initializing its storage. With `createBundle(io, { suspense: true })`, this hook waits until storage is loaded and returns `[TData, SharedType]` instead.
...
const { useStorageField } = createBundle(io);
