...
Bind your server type as a type argument, then pass presence, storage, metadata, and endpoints to `.config()`. Do not pass options to `createClient` itself — that would freeze presence and storage inference.
...
const io = createClient<typeof ioServer>().config({
  // Optional: Define the storage schema and an optional JSON seed.
  storage: yjs.storage({
    schema: yjs.schema({
