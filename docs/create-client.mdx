...
| Storage schema                                          | Client   | npm install @pluv/crdt                |
...
export const client = createClient<typeof ioServer>().config({
  // Optional: Define the storage schema for rooms. This sets
  // the types for storage when using it in your application.
  // `initialStorage` is an optional JSON seed and can be overwritten
  // when creating a room via `client.createRoom`.
  storage: yjs.storage({
    schema: yjs.schema({
