...
<img width="80%" src="https://raw.githubusercontent.com/pluv-io/pluv/refs/heads/master/assets/create_api_keys.png" alt="Create api key" loading="lazy" />
...
<img width="80%" src="https://raw.githubusercontent.com/pluv-io/pluv/refs/heads/master/assets/create_webhook_dashboard.png" alt="Create webhook dashboard" loading="lazy" />

Note that the endpoint must be publicly accessable (i.e. not localhost), so we recommend using [Ngrok](https://ngrok.com/) or [Cloudflare Tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/) for testing.

<img width="48%" src="https://raw.githubusercontent.com/pluv-io/pluv/refs/heads/master/assets/create_webhook_dialog.png" alt="Create webhook dialog" loading="lazy" />
...
For `@pluv/platform-pluv` specifically, authorization is required unlike in the other platforms where it is optional. Setting this up is not any different between the other platforms. See [Authorization](/docs/io/authorization) for more detailed documentation.
...
export const GET = async (request: Request) => {
    ...
    // If authorization succeeds, return the token as a text response with a
    // 200 status code
...
The example below consists of all 4 event handlers that are supported for `@pluv/platform-pluv`.
...
export const ioServer = io.server({
    ...
    // Rooms are destroyed when they are empty (i.e. no more active connections)
    ...
    // Optional: This handler is called after a new connection is established
...
// Note that the `wsEndpoint` param is omitted when using `@pluv/platform-pluv`
...
// properties of objects within server components (e.g. you are unable to write
// pluv.PluvRoomProvider in a layout that is a server component)
export const {
    PluvProvider,
    PluvRoomProvider,
