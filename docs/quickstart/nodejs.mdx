...
description: "Self-host pluv.io on Node.js and build a type-safe realtime WebSocket API."
---
...
// Note: All event listeners are configured at the server level via io.server(),
// not when creating individual rooms. See the ioServer configuration above.

const getRoom = (roomId: string): InferIORoom<typeof ioServer> => {
  const existing = rooms.get(roomId);
  if (existing) return existing;

  // Rooms are created without listener options - all listeners are server-level
...
// Handle room cleanup in the server-level onRoomDestroyed listener
// (configured in io.server() above)
