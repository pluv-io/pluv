...
Then, on the frontend, define a `loro.schema` and pass it to `createClient` via `loro.storage`. JSON seeds go in `initialStorage`.
...
const io = createClient<typeof ioServer>().config({
  storage: loro.storage({
    schema: loro.schema({
      // Top-level fields must be Loro CRDT nodes (loroList, loroMap, loroText, …)
      groceries: loro.loroList(s.string()),
    }),
  }),
  initialStorage: {
    groceries: [],
...
const io = createClient().config({
  ...
  storage: loro.storage({
    schema: loro.schema({
      groceries: loro.loroList(s.string()),
      // Top-level LoroMap named "chats" that nests LoroText items
      chats: loro.loroMap(loro.loroText()),
    }),
  }),
  initialStorage: {
    groceries: [],
