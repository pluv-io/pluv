---
title: "@pluv/client"
description: "API reference for @pluv/client: create clients, rooms, event routers, and interact with presence and storage."
---

## createClient

Returns a builder whose `.config()` method creates a `PluvClient`

Bind your server type as a type argument, then pass the shared `treaty`, metadata, and endpoints to `.config()`. Do not pass options to `createClient` itself. That would freeze inference.

### createClient basic example

Bind your server type and pass the shared treaty. Hosted pluv uses `publicKey`; self-hosted uses `authEndpoint`.

See the [Create Client](/docs/guides/create-client) docs for more details.
...
See the [Create Rooms](/docs/guides/rooms/client) docs for more details.

```ts
export const io = createClient().config({
  treaty,
  initialStorage: {
    messages: [],
  },
...
const room = io.createRoom("my-example-room", {
  // Required if presence is defined on the treaty
  initialPresence: {
    selectionId: null,
  },
...
Creates a new type-safe event procedure to be broadcasted from `PluvClient`. See [Define Events](/docs/guides/events/client) for more information.
...
Creates a new type-safe event procedure router to attach events and connect to a `PluvRoom` via `PluvRoom.createRoom`. See [Define Events](/docs/guides/events/client) for more information.
...
Returns a user info object containing the authorized user `data` and the user's presence.
...
Returns a user info object for the given user id (`data.id`).

```ts
const other = room.getOther(userId);
```

### PluvRoom.getOtherByConnectionId

Returns `UserInfo | null`

Resolves a connection id to the person that owns it.

```ts
const other = room.getOtherByConnectionId(connectionId);
```
...
Returns all user info objects for other people in the room.

```ts
const others = room.getOthers();
```

### PluvRoom.getRoomStats

Returns `RoomStats`

Live socket count and unique-user count for the room.
...
On-demand identity page (no presence). Forward-only cursor paging. Includes yourself. Grouped by `data.id`. `limit` defaults to 50 and must be an integer from 1 to 100. The promise always settles: check `success` before reading `users`. `FAILED` is a timeout (or a dropped request while the socket is open). `NOT_CONNECTED` is returned immediately when the room is not connected.

```ts
const result = await room.listUsers({ cursor: null, limit: 50 });

if (!result.success) {
  // result.error.code: "FAILED" | "INVALID_LIMIT" | "NOT_CONNECTED"
...
Returns the native CRDT shared type, or `null` while storage is unavailable. For `@pluv/crdt-yjs`, this is a Yjs type such as `Y.Array`.
...
const room = io.createRoom("my-example-room", {
  ...
  // Override the JSON seed defined in createClient
  initialStorage: {
    messages: ["hello world!"],
  },
});
...
### PluvRoom.subscribe.error

Returns `() => void`

Fires when the room reports a failure (procedure throw, size limit, rejected register). This is not the WebSocket `error` event from `room.addEventListener("error")`.
...
```ts
const unsubscribe = room.subscribe.other(userId, (userInfo) => {
  const { data, presence } = userInfo;
...
### PluvRoom.subscribe.roomStats

Returns `() => void`

Subscribes to live room connection/user counts.

```ts
const unsubscribe = room.subscribe.roomStats((stats) => {
  console.log(stats.connectionCount, stats.userCount);
});
```
...
const unsubscribe = room.subscribe.storage.messages((messages: string[]) => {
  ...
  // If the native shared type is needed, call getStorage here.
