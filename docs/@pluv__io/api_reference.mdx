...
Creates a server-side `PluvIO` client that can be used to create event procedures, routers and servers. Below will show an example configuration for `createIO`. To learn more about `createIO`, read [Authorization](/docs/io/authorization), [Cloudflare Workers](/docs/quickstart/cloudflare-workers), [Node.js](/docs/quickstart/node.js) and [PubSub](/docs/io/pubsub).
...
Registers a websocket to the room. Accepts a secondary options parameter to pass-in a token for authorizing the room. Read [Cloudflare Workers](/docs/quickstart/cloudflar-workers), [Node.js](/docs/quickstart/nodejs) and [Authorization](/docs/io/authorization) to learn more.
