...
- 894370a: Loaded room storage is no longer overwritten when another client joins with `initialStorage`.
...
- 8ebe119: Avoid stale Yjs docs and racing storage writes.
...
- 50c1353: Updated the `PluvYjsAwareness` class so that the readonly `meta` property is type-compatible with the latest blocknote version.
