...
- 8806f35: Include built `dist/` artifacts in published npm packages via `"files": ["dist"]`. `dist` is gitignored, so without this field npm omitted the build output from 5.0.1 tarballs (packages shipped `src/` only).
